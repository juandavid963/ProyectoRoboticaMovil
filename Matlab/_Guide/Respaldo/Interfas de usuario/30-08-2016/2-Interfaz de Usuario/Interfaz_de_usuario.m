...
function Interfaz_de_usuario_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    Ui_ori_1=0; E_ori_1=0; Kp_ori=2; Ti_ori=10000; Td_ori=0;
    Ui_lat_1=0; E_lat_1=0; Kp_lat=5; Ti_lat=2; Td_lat=0;
    Ui_lon_1=0; E_lon_1=0; Kp_lon=4; Ti_lon=2; Td_lon=0;
            ...
            if (cputime-tiempo_inicial > 2 && n_puntos_ref~=0) % espero 2 segundos mientras se estabilza la camara
                if(get(handles.start_radiobutton,'Value')==1)
                    ...
                    if(control_automatico==1 && robot_encontrado==1) % si oprimio el boton de control automatico                        
                        ...
                        if(abs(centro_x-pixel_x(k))*factor_cm_px<5 && abs(centro_y-pixel_y(k))*factor_cm_px<5 && abs(orientacion_corregida-orientacion_tabla(k))<5)
                           control_activar=0;
                           tiempo_cambio=cputime;
                           k=k+1;  
                           Ui_lat_1=0; % borro la memoria del integrador
                           Ui_lon_1=0;
                           Ui_ori_1=0;
                        ...
                        if (control_activar==1)
                            ...
                            if (abs(Dla_cm)<5 && abs(Dlo_cm)<5)
                                ...
                                %control de estiba
                                if (abs(orientacion_corregida-orientacion_tabla(k))<3)
                                    if(estiba(k)==1) longitudinal=124; lateral=124; orientacion=124; end %montacarga baja
                                    if(estiba(k)==0) longitudinal=123; lateral=123; orientacion=123; end %montacarga sube                                                                        
                                end
                            end
                        else
                            orientacion=0;
