...
function Interfaz_de_usuario_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    Ui_ori_1=0; E_ori_1=0; Kp_ori=4; Ti_ori=1000; Td_ori=0;
    Ui_lat_1=0; E_lat_1=0; Kp_lat=4; Ti_lat=3; Td_lat=0;
    Ui_lon_1=0; E_lon_1=0; Kp_lon=3; Ti_lon=3; Td_lon=0;
...
function start_radiobutton_Callback(hObject, eventdata, handles)
    ...
    if(get(handles.start_radiobutton,'Value')==1 && fin_proceso==1) 
        ...
        % variables de tiempo
        bandera_siguiente_punto=0;
        bandera_control_orientacion=0;
        bandera_control_estiba=0;
        bandera_estiba=0;
            ...
            if (cputime-tiempo_inicial > 2 && n_puntos_ref~=0) % espero 2 segundos mientras se estabilza la camara
                if(get(handles.start_radiobutton,'Value')==1)
                    ...
                    if(control_automatico==1 && robot_encontrado==1) % si oprimio el boton de control automatico                        
                        ...
                        if (bandera_control_orientacion==0)
                            lateral=control_lateral(Dla_cm);
                            longitudinal=control_longitudinal(Dlo_cm);
                        end
                        if (abs(Dla_cm)<3 && abs(Dlo_cm)<3), bandera_control_orientacion=1; end
                        if(bandera_control_orientacion==1 && bandera_control_estiba==0)
                            orientacion=control_orientacion(orientacion_tabla(k)-orientacion_corregida);
                            lateral=0;
                            longitudinal=0;
                            
                            %control de estiba
                            if (bandera_control_estiba==0 && abs(centro_x-pixel_x(k))*factor_cm_px<3 && abs(centro_y-pixel_y(k))*factor_cm_px<3 && abs(orientacion_corregida-orientacion_tabla(k))<3)
                                tiempo_estiba=cputime; 
                                bandera_control_estiba=1;
                            end                                                                                      
                        end
                        if (bandera_control_estiba==1)
                            if(cputime-tiempo_estiba<5)
                        ...
                        % activar bandera para pasar al siguiente punto
                        if(bandera_siguiente_punto==0 && bandera_estiba==1 && abs(centro_x-pixel_x(k))*factor_cm_px<3 && abs(centro_y-pixel_y(k))*factor_cm_px<3 && abs(orientacion_corregida-orientacion_tabla(k))<3)                                                      
                            bandera_siguiente_punto=1; 
                            tiempo_cambio=cputime;
                        end 
                        
                        % si durante los 5 segundos de espera se sale de la referencia
                        if(abs(centro_x-pixel_x(k))*factor_cm_px>3 && abs(centro_y-pixel_y(k))*factor_cm_px>3 && abs(orientacion_corregida-orientacion_tabla(k))>3)
                            bandera_siguiente_punto=0;
                        end
                        
                        disp([abs(centro_x-pixel_x(k))*factor_cm_px abs(centro_y-pixel_y(k))*factor_cm_px abs(orientacion_corregida-orientacion_tabla(k)) bandera_siguiente_punto]);
                        
                        % cambiar referencia
                        if(cputime-tiempo_cambio>2 && bandera_siguiente_punto==1)
                           k=k+1;
                           bandera_siguiente_punto=0;
                           bandera_control_orientacion=0;
                           bandera_estiba=0;
