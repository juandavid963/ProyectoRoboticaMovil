            ...
            if (cputime-tiempo_inicial > 2 && waypoint~=0) % espero 2 segundos mientras se estabilza la camara
                if(get(handles.start_radiobutton,'Value')==1)
                    ...
                    if(control_automatico==1 && robot_encontrado==1) % si oprimio el boton de control automatico                        
                        ...
                        % control lateral
                        lateral=control_lateral();
                        % control longitudinal
                        longitudinal=control_longitudinal();
                        % control en orientacion
                        orientacion=control_orientacion();
                        
                        % mientras no esté lo suficientemente cerca no puede pasar a la siguiente referencia
                        if(abs(centro_x-pixel_x(k))<5 && abs(centro_y-pixel_y(k))<5 && abs(orientacion_corregida-orientacion_tabla(k))<5)
                           k=k+1;
                           if(k>waypoint) k=1; end % aqui termina el recorrido
                        end                    
                    end                                                            
                end
            end
...
function control_lateral(handles)
j=1;

function control_longitudinal(handles)
j=1;

function control_orientacion(handles)
j=1;
