...
% Last Modified by GUIDE v2.5 07-Aug-2016 16:37:21
...
boton_1_montacarga=0;
boton_2_montacarga=0;
rx=0;
...
if(get(handles.start_radiobutton,'Value')==1 && fin_proceso==1)
    ...
    try
        ...
        %errordlg('Ocurrio un Error al abrir el PS','Error',mode);      
        set(handles.start_radiobutton,'Value',0);            
        return;                
    end    
    
    % desavilita los "popmenu" y el boton de "imagen de referncia"
    ...
    orientacion_buffer=ones(1,10);
    longitudinal_buffer=ones(1,10);
    lateral_buffer=ones(1,10); 
    rx_buffer=ones(1,10);
    A=[0;0;0];
...
%------------%%------------%%------------%%------------%%------------%
    ...
    while get(handles.start_radiobutton,'Value')==1        
        ...
        set(handles.text_umbral,'String',umb); % poner valor del humbral en el text 
        bw=im2bw(img_gris,umb);  %encuentro imagen a blanco y negro 
        area_imagen=size(bw,1)*size(bw,2);
                   
        % etiqueta elementos conectados de la iamgen a blanco y negro
        [L1 N1]=bwlabel(bw);   % para los contornos blancos
        [L2 N2]=bwlabel(~bw);  % para los contornos negros
        % calcular propiedades de los objetos 
        contornos_1=regionprops(L1,'BoundingBox','Area','Centroid','MajorAxisLength','MinorAxisLength');   % encuentro propiedades de los contoronos blancos    
        contornos_2=regionprops(L2,'BoundingBox','Area','Centroid','MajorAxisLength','MinorAxisLength');   % encuentro propiedades de los contoronos negros 
        contornos=[contornos_1; contornos_2];    % unifico todos los contornos (blancos y negros)
                ...
                if(contornos(i).BoundingBox(3)*contornos(i).BoundingBox(4) < area_imagen*0.9) % excluir contornos grandes, que puedan contener otros contornos en su interior
                    ...
                    for j=1:length(contornos)
                        if(contornos(j).Area > 3)% ruido de contorno interior   
                            if(contornos(j).BoundingBox(1)>x1 && contornos(j).BoundingBox(1)<x2) % identifico los contornos dentro del cuadrado
                                if(contornos(j).BoundingBox(2)>y1 && contornos(j).BoundingBox(2)<y2)
                                    if(contornos(j).Centroid(1)>x1 && contornos(j).Centroid(1)<x2)
                                        if(contornos(j).Centroid(2)>y1 && contornos(j).Centroid(2)<y2)
                    ...
                    if (contador_contornos==2) % si reconoce el patron con 2 contornos internos   
                        ...
                        color=get(handles.patron_popupmenu,'Value');
        ...
        orientacion_buffer(10)=orientacion;       
        longitudinal_buffer(10)=longitudinal;       
        lateral_buffer(10)=lateral;               
        axes(handles.axes9); plot([0 9],[0 0],(0:9),orientacion_buffer,(0:9),longitudinal_buffer,(0:9),lateral_buffer); axis([0 10 -150 150]);
...
%         end
%         rx_buffer(10)=rx;
%         axes(handles.axes12); plot((0:9),rx_buffer); axis([0 10 -250 250]);
%         set(handles.text21,'String',rx);
%         B=[B rx];
        ...
        axes(handles.axes8); plot([0 19],[tiempo tiempo],(0:19),tiempo_real); axis([0 20 0 0.1]);
      
        tiempo_real(20)=toc; 
        while tiempo_real(20)<tiempo                      
            tiempo_real(20)=toc;
        end             
               
        tic      
        set(handles.text7,'String',tiempo_real(20)*1000);
    end    
%------------%%------------%%------------%%------------%%------------%   
    ...
    % envia señal de stop (mirar si es necesario)
    if (PS~=0) 
    try
    fwrite(PS,255);
    fwrite(PS,encriptar(0));
    fwrite(PS,encriptar(0));
    fwrite(PS,encriptar(0));
    end
    end
