...
% Last Modified by GUIDE v2.5 30-Aug-2016 19:18:40
...
function Interfaz_de_usuario_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    Ui_ori_1=0; E_ori_1=0; Kp_ori=2.5634; Ti_ori=65.9152; Td_ori=0;
    Ui_lat_1=0; E_lat_1=0; Kp_lat=1.3; Ti_lat=3; Td_lat=0;
    Ui_lon_1=0; E_lon_1=0; Kp_lon=1.2; Ti_lon=3; Td_lon=0;
...
function start_radiobutton_Callback(hObject, eventdata, handles)
    ...
    if(get(handles.start_radiobutton,'Value')==1 && fin_proceso==1) 
        ...
        set(handles.patron_popupmenu,'Enable','off');
        ...
        %iniciaclizo variables de control para visualizar
        orientacion_buffer=ones(1,30);
        longitudinal_buffer=ones(1,30);
        lateral_buffer=ones(1,30); 

        % Variables de orientacion real y continua del robot  
        orientacion_mem=0;
        contador_vuelta=0;
        ...
        tiempo_proceso_1=cputime;
        tiempo_proceso_2=cputime;
        tiempo_proceso_3=cputime;
        tiempo_proceso_4=cputime;
        tiempo_proceso_5=cputime;
        

        tiempo_inicial=cputime; % inicio para el control
        tiempo_cambio=cputime;         % tiempo para detectar cambio
        tiempo_real=0.1*ones(1,30); % vector que guarda el tiempo pasado para visualizar
        tic;
    ...
    %------------%%------------%%------------%%------------%%------------%
        ...
        while get(handles.start_radiobutton,'Value')==1              
            ...
            % variable bandera para saber si en el ciclo encontro el patrón 
            robot_encontrado=0;
            ...
            % Analisis de los contornos para encontrar patron rectangular
            for i=1:length(contornos)
                if (contornos(i).Area > 20) % para filtrar ruido de contronos muy pequeños                      
                    if(contornos(i).BoundingBox(3)*contornos(i).BoundingBox(4) < area_imagen*0.8) % excluir contornos grandes, que puedan contener otros contornos en su interior                        
                        factor_lados=contornos(i).MajorAxisLength/contornos(i).MinorAxisLength;
                        if (factor_lados>1.5 && factor_lados<2.5)                                          
                            factor_1=10/contornos(i).MinorAxisLength;  %8+1+1=10
                            Area_Rectangulo_1=contornos(i).Area*factor_1*factor_1;
                            if(Area_Rectangulo_1>90 && Area_Rectangulo_1<130)  % area real 116                                
                                factor_2=20/contornos(i).MajorAxisLength;
                                Area_Rectangulo_2=contornos(i).Area*factor_2*factor_2;
                                if(Area_Rectangulo_2>90 && Area_Rectangulo_2<140) % area real 116                                        
                                    ...
                                    for j=1:length(contornos)   % analiso los contornos internos                                        
                                        if(contornos(j).Area > 2) % ruido de contorno interior 
                                                ...
                                                if(contornos(j).BoundingBox(2)>y1 && contornos(j).BoundingBox(2)<y2) % identifico los contornos dentro del cuadrado por BoundinBox
                                                    if(contornos(j).Centroid(1)>x1 && contornos(j).Centroid(1)<x2) % identifico los contornos dentro del cuadrado por Centroid
                                                        if(contornos(j).Centroid(2)>y1 && contornos(j).Centroid(2)<y2) % identifico los contornos dentro del cuadrado por Centroid                                                            
                                                            relacion_lados=contornos(j).BoundingBox(3)/contornos(j).BoundingBox(4);
                                                            if(relacion_lados>1), relacion_lados=1/relacion_lados; end
                                                            if(relacion_lados>0.5)                                                                 
                                                                distancia_centro=sqrt((contornos(i).Centroid(1)-contornos(j).Centroid(1))^2+(contornos(i).Centroid(2)-contornos(j).Centroid(2))^2);                                                                
                                                                if (distancia_centro*factor_1*factor_1 < 8) %real 3                                                                    
                                                                    if (bw(floor(contornos(j).Centroid(2)),floor(contornos(j).Centroid(1)))==(color-1))                                                                         
                                                                        Area_circulo_1=contornos(j).Area*factor_1*factor_1;                                                                            
                                                                        if(Area_circulo_1>2 && Area_circulo_1<20) % filtro de patron por area real                                                                            
                                                                            Area_circulo_2=contornos(j).Area*factor_2*factor_2;
                                                                            if(Area_circulo_2>2 && Area_circulo_2<20) % filtro de patron por area real
                                                                                rectangle('Position',[contornos(j).Centroid 3 3],'EdgeColor','y','LineWidth',3);
                                                                                contador_contornos_internos=contador_contornos_internos+1; % cuento contornos
                                    ...
                                    if (contador_contornos_internos==2) % si reconoce el patron con 2 contornos internos   
            ...
            % Control Automatico identificar distancias                               
            ...
            if (cputime-tiempo_inicial > 2 && n_puntos_ref~=0) % espero 2 segundos mientras se estabilza la camara
                if(get(handles.start_radiobutton,'Value')==1 && get(handles.manual_checkbox,'value')==0)
                    % habilito el control automatico
                    set(handles.control_pushbutton,'Enable','on'); 
                    if(control_automatico==1 && robot_encontrado==1) % si oprimio el boton de control automatico y el robot es encontrado en el procesamineto de imagen                       
                        ...
                        % fin proceso 1
                        if (bandera_proceso_1==0 && (abs(Dla_cm)>4 || abs(Dlo_cm)>4))
                            tiempo_proceso_1=cputime;                            
                        end                        
                        if (cputime-tiempo_proceso_1>3)
                            bandera_proceso_1=1;
                        end
                        ...
                        % proceso 2
                        if(bandera_proceso_1==1 && bandera_proceso_2==0)
                        ...
                        % fin proceso 2
                        if (bandera_proceso_1==1 && bandera_proceso_2==0 && abs(orientacion_tabla(k)-orientacion_corregida)>4)
                            tiempo_proceso_2=cputime;                            
                        end   
                        if (bandera_proceso_1==1 && bandera_proceso_2==0 && cputime-tiempo_proceso_2>3)
                            bandera_proceso_2=1;
                        end                        
                        
                        
                        % proceso 3
                        if(bandera_proceso_2==1 && bandera_proceso_3==0)
                            orientacion=control_orientacion(orientacion_tabla(k)-orientacion_corregida);
                            lateral=control_lateral(Dla_cm);
                            longitudinal=control_longitudinal(Dlo_cm);
                        end        
                        ...
                        % fin proceso 3
                        if (bandera_proceso_2==1 && bandera_proceso_3==0 && (abs(Dla_cm)>4 || abs(Dlo_cm)>4 || abs(orientacion_tabla(k)-orientacion_corregida)>4))
                            tiempo_proceso_3=cputime;                            
                        end   
                        if (bandera_proceso_2==1 && bandera_proceso_3==0 && cputime-tiempo_proceso_3>3)
                            bandera_proceso_3=1;
                            tiempo_estiba=cputime;
                        end                                                 
                        
                        % proceso 4
                        if (bandera_proceso_3==1 && bandera_proceso_4==0)
                            if (k>1)
                                if (estiba(k)-estiba(k-1)~=0)
                                    if(cputime-tiempo_estiba<4)                                
                                        if(estiba(k)==0), longitudinal=124; lateral=124; orientacion=124; end %montacarga baja
                                        if(estiba(k)==1), longitudinal=123; lateral=123; orientacion=123; end %montacarga sube 
                                    else
                                        bandera_proceso_4=1;
                                    end
                                else
                        ...
                        % fin proceso 4
                        if (bandera_proceso_4==1 && bandera_proceso_5==0)
                            bandera_proceso_5=1; 
                        ...
                        disp([abs(centro_x-pixel_x(k))*factor_cm_px abs(centro_y-pixel_y(k))*factor_cm_px abs(orientacion_corregida-orientacion_tabla(k)) bandera_proceso_1 bandera_proceso_2 bandera_proceso_3 bandera_proceso_4 bandera_proceso_5]);
                        
                        % proceso 5 - cambiar referencia
                        if(cputime-tiempo_cambio>2 && bandera_proceso_5==1)
                           k=k+1;
                           bandera_proceso_1=0;
                           bandera_proceso_2=0;
                           bandera_proceso_3=0;
                           ...
                           % borro la memoria del integrador
                           Ui_lat_1=0; 
                           Ui_lon_1=0;
                           Ui_ori_1=0;
                    ...
                    else % si no he oprimido el boton o el ciclo se ha acabado o no encuentro el robot
                        longitudinal=0;
                        lateral=0;
                        orientacion=0;                        
                    end                                                            
            ...
            if(get(handles.manual_checkbox,'value')==1) % si el modo manual está activado
                ...
                estiba=round(get(handles.estiba_slider,'value'));
                if(estiba==+1), longitudinal=123; lateral=123; orientacion=123; end
                if(estiba==-1), longitudinal=124; lateral=124; orientacion=124; end
                ...
                if (longitudinal==0 && lateral==0 && orientacion==0 && ~isempty(joys))
                    ...
                    if(boton_1_montacarga==1), longitudinal=124; lateral=124; orientacion=124; end %montacarga baja
                    if(boton_2_montacarga==1), longitudinal=123; lateral=123; orientacion=123; end %montacarga sube
                end
                ...
                set(handles.estiba_text,'string',estiba);
            end
...
%             end
    ...
    %------------%%------------%%------------%%------------%%------------%   
        ...
        set(handles.patron_popupmenu,'Enable','on');
...
Ut_ori=Up_ori+Ui_ori;

% limites de control
if Ut_ori>+127, Ut_ori=+127; end
if Ut_ori<-127, Ut_ori=-127; end

Ui_ori=Ut_ori-Up_ori;
...
set(handles.uitable1,'ColumnWidth',{round(0.2*ancho_tabla) round(0.2*ancho_tabla) round(0.3*ancho_tabla) round(0.3*ancho_tabla)});
...
set(handles.estiba_slider,'enable','on');
...
set(handles.estiba_text,'enable','off');
end
...
% --- Executes on button press in estiba_button_up.
function estiba_button_up_Callback(hObject, eventdata, handles)
% hObject    handle to estiba_button_up (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in estiba_button_down.
function estiba_button_down_Callback(hObject, eventdata, handles)
% hObject    handle to estiba_button_down (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on slider movement.
function estiba_slider_Callback(hObject, eventdata, handles)
set(handles.estiba_slider,'value',0);
% hObject    handle to estiba_slider (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes during object creation, after setting all properties.
function estiba_slider_CreateFcn(hObject, eventdata, handles)
% hObject    handle to estiba_slider (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
